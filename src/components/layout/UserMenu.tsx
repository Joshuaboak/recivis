        ...
        {menuOpen && (
          ...
          >
            ...
            {/* Guided tutorial — a setting belonging to this person, not to
                ...
                if the save fails. */}
            <div className="py-1 border-b border-border-subtle flex items-center gap-2 px-4 py-2.5 text-sm text-text-secondary">
              <button
                className="flex items-center gap-3 flex-1 min-w-0 text-left hover:text-text-primary transition-colors cursor-pointer"
                onClick={async () => {
              ...
              >
                ...
                <span className="flex-1 truncate">Guided tutorial</span>
                {/* The switch centres its knob with padding rather than with
                    absolute offsets, so both ends have the same gap and the
                    knob cannot sit proud of the track. */}
                <span
                  className={`w-9 h-5 p-0.5 rounded-full flex items-center transition-colors flex-shrink-0 ${
                    tutorialOn ? 'bg-csa-accent' : 'bg-border'
                  }`}
                >
                  <span
                    className={`w-4 h-4 rounded-full bg-white transition-transform ${
                      tutorialOn ? 'translate-x-4' : 'translate-x-0'
                    }`}
                  />
                </span>
              </button>
